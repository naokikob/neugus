...
let rec read_options index =
  ...
  | "-learn" -> (learnopt := Sys.argv.(index+1)^(!learnopt); read_options (index+2))
...
let rec repeat smtfile datfile =
  let (smtfile2,datfile2) =
    if !datagen2>0 then
          ...
          let _ = Sys.command ("nkhoice --simplify off -v -v --datagen 100 --datafile "
          ...
          if successful(hoicedatalog) then
            ...
            let _ = Sys.command (!path^"hoicelog2dat.sh "^hoicedatalog) in
  ...
  let option =
    "-qce 2 -distill -smt "^qualfile^" -4l -nodes "^(string_of_int !size1)^" -nodes2 "^(string_of_int !size2)^" -bias "^(string_of_int !bias)^" -rate "^(!rate)^" "
    ^(if !cycle mod 3=0 then "-conjqual 3 " else "")
    ^(if !cycle mod 2=0 then "-coeff 5 " else "")
    ^(!learnopt)
  in
  let _ = print_string ("Learning qualifiers with option: "^option^"\n"); flush stdout in
  let _ = Sys.command (!path^"learn.sh "^option^" "^datfile2) in
  ...
  let _ = Sys.command ("nkhoice --simplify off --preproc off --datafile "^datfile2^" -v -v --timeout "^(string_of_int !timelimit)^" "^hintfile^" > "^hoicelog) in 
...
let main () =
  ...
  let _ = print_string (!path^"\n") in
  let tmpfile = smtfile^".out" in
  if !datagen2>0 then
    let _ = print_string "running hoice preprocessor\n"; flush stdout in
    let _ = Sys.command ("nkhoice --infer off --log_preproc on -v -v "^smtfile^" > "^tmpfile) in
    let _ = print_string "done\n"; flush stdout in
    let optsmtfile = "hoice_out/preproc/instances/preproc_0000_fixed_point.smt2" in
    let _ = print_string "collecting data by using hoice --datagen\n";flush stdout in
    let hoicedatalog = "hoicedata.out" in
    let _ = Sys.command ("nkhoice --simplify off --preproc off -v -v --datagen "^(string_of_int !datagen2)
    ...
    let _ = Sys.command ("nkhoice --timeout "^(string_of_int !timelimit)^" --log_preproc on -v -v "^smtfile^" > "^tmpfile) in
    let _ = print_string "done\n"; flush stdout in
